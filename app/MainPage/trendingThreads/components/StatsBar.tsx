  ...
  return (
    <div className="rounded-lg border border-[var(--border-soft)] bg-[var(--bg-surface)] p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[13px] uppercase tracking-widest font-bold text-[var(--text-secondary)]">Forum Statistics</span>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {items.map(stat => (
          <div key={stat?.label} className="bg-[var(--bg-page)] rounded-md px-3 py-2.5 flex items-center gap-2.5">
            <div className="text-[var(--accent)]">{stat?.icon}</div>
            <div>
              <div className="text-[var(--text-primary)] font-bold text-base leading-none">{stat?.value}</div>
              <div className="text-[var(--text-secondary)] font-semibold text-[12px] mt-0.5">{stat?.label}</div>
            </div>
          </div>
        ))}
      </div>
      <div className="mt-3 pt-3 border-t border-[var(--border-soft)]">
        <p className="text-[var(--text-secondary)] text-[13px]">
          Newest member:{' '}
          <a href={`/u/${stats?.newestMember}`} className="text-[var(--accent)] text-sm font-semibold hover:underline">
            {stats?.newestMember}
          </a>
        </p>
      </div>
