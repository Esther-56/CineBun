...
export default function SubforumHeader({ subforum, type, length }: SubforumHeaderProps) {
  ...
  // Staff can post even in read-only subforums; everyone else needs it to be non-read-only.
  const canThread = !!(subforum.isReadOnly === false || snap.role?.permissions?.canAccessAdmin);
  const showNewThreadButton = !!id && canThread && (type === 'threads' || length);
